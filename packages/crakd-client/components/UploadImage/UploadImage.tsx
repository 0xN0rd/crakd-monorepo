...
const UploadImage: FC<UploadImageProps> = ({ isCover, setIsLoading }) => {
    ...
    const handleChange = async (e: ChangeEvent) => {
        ...
        if (file.size >= MaxImageSize.Image) {
            alert(`File size should be less than ${MaxImageSize.Image / 1000000}MB`);
            return;
        }
        try {
            ...
            console.error('An error eccurred while uploading an image: ', error);
        }
