...
const UserController = {
    ...
    newUsers: async (req: Request, res: Response): Promise<any> => {
        const authUser = req.user as AuthUser;
        const users = await getNewUsers(authUser?._id);
        return res.send(users);
    },
    ...
    banUser: async (req: Request, res: Response): Promise<any> => {
        ...
        if (user.role === UserRole.SuperAdmin) {
            return res.status(ErrorCodes.Bad_Request).send(`You cannot ban SuperAdmin users!`);
        }
