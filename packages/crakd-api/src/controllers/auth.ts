...
const AuthController = {
    ...
    signUp: async (req: Request, res: Response, next: NextFunction): Promise<any> => {
        const { fullName, username, email, gamertag, password } = req.body;

        if (!fullName || !username || !email || !password || !gamertag) {
        ...
        const user = await createUser(fullName, username, gamertag, email, password, true);
        ...
        if (isEmailVerificationRequired) {
            try {
                const template = await getEmailTemplate({
                    ...
                    description: `Thank you for signing up. To complete your registration, pease confirm your email.`,
                    ctaLink: `${req.headers.origin}/email-verify?email=${email}&token=${token}`,
                    ctaText: 'Confirm email',
                });
                await sendEmail({
                    to: email,
                    subject: 'Crakd Account - Email Verification',
                    html: template,
                });
    ...
    emailVerify: async (req: Request, res: Response, next: NextFunction): Promise<any> => {
        passport.authenticate('signup', { session: false }, async (err) => {
            ...
            if (!token || !email) {
                return res.status(ErrorCodes.Bad_Request).send('Email verifiation has been failed due to invalid arguments.');
            }
    ...
    forgotPassword: async (req:Request, res: Response): Promise<any> => {
        ...
        if (!user || (user && isEmailVerificationRequired && !user.emailVerified)) {
            return res.status(ErrorCodes.Bad_Request).send('A user with a given email address doesn not exist.');
        }
        ...
        const template = await getEmailTemplate({
            ...
            description: `We received a request to reset your password. You may click the button below to choose your new password. If you did not make this request, you can safely ignore this message.`,
    ...
    resetPassword: async (req: Request, res: Response): Promise<any> => {
        ...
        try {
            ...
            return res.status(ErrorCodes.Bad_Request).send('The link ahs expired.');
        }
