...
export const getUserByGamertag = async (gamertag: string): Promise<any> => {
  const user = await (await User.findOne({ gamertag })).isSelected('-password');
  return user;
};
...
export const onlineUsers = async (userId?: string): Promise<any> => {
  const users = await User.find({ isOnline: true, _id: { $ne: userId }, banned: { $ne: true } }).select('-password');
  return users;
};

export const getNewUsers = async (userId?: string): Promise<any> => {
